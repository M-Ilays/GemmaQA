"""Build Strands agent for AgentCore deployment with HTTP tools."""
...
# Import HTTP-based tools instead of local ones
from app.strands_agent.http_tools import (
...
AWS Strands Agents SDK and deployed on Amazon Bedrock AgentCore Runtime.
...
Technical context:
- You are running on AgentCore Runtime (serverless, managed by AWS)
- Your tools make HTTP calls to the GemmaQA backend API
- The backend runs AgentController + Playwright to do the actual testing
"""


def build_agentcore_agent(*, callback_handler: Any = None) -> Any:
    """Create a Strands Agent for AgentCore deployment with HTTP-based tools."""
    ...
    # Check for backend URL (required for AgentCore deployment)
    backend_url = os.getenv("GEMMAQA_BACKEND_URL")
    if not backend_url:
        raise RuntimeError(
            "AgentCore deployment requires GEMMAQA_BACKEND_URL "
            "(HTTP endpoint for the GemmaQA backend API)"
        )
    ...
    return Agent(
        name="gemmaqa_strands_coordinator_agentcore",
        model=model,
